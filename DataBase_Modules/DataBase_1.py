...
def Create_Database(sql_file_path: str, host: str, user: str, password: str, database_name: str) :
    try : 
        ...
        # 执行命令并从 SQL 文件中创建数据库
        result = subprocess.run(command, input=sql_content, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, shell=True)
...
class DataBase() :
    ...
    def Insert_Detected_Image_Data(self, image_path: str, defect_result_path: str,
        ...
        try :
            ...
            INSERT INTO detection_results (
                image_name, image_type, image_data, defect_result, upload_time, defect_type, defect_area_proportion
            ) VALUES (
                %s, %s, %s, %s, %s, %s, %s
            ...
            """
            self.cursor.execute(sql, (image_name, image_type, image_data, defect_result,\
    ...
    def Get_Detected_Image(self, image_ids: list, save_path: str) :
        try : 
            ...
            sql = f"""
            SELECT image_name, image_type, image_data, defect_result, defect_type, defect_area_proportion
            ...
            for result in results:
                # 查询的每一条结果
                image_name, image_type, image_data, defect_result, defect_type, defect_area_proportion = result
                image_file_path = f'{save_path}/{image_name}'
                with open(image_file_path, "wb") as file :
                    file.write(image_data)
                
                detec_result_file_path = f'{save_path}/{image_name.split(f".{image_type}")[0]}_result.png'
                with open(detec_result_file_path, "wb") as file :
    ...
    def Get_Annotation_Data(self, image_ids: list, save_path: str) :
        try : 
            ...
            for result in results :
                ...
                print(f'图片已保存为 {image_name}, 缺陷标注已保存为 {annotation_image_path}, yolo标注结果保存为 {yolo_anno_path}')
...
if __name__ == "__main__" :
    # Create_Database('./build.sql', 'localhost', 'root', '200407', database_name='new')
    
    
    db = DataBase("localhost", "root", "200407", "new",
    ...
    db.Insert_Detected_Image_Data(image_path='1234.png', defect_result_path='1234.png',
                                defect_type='dent, crak', defect_proportion='dent:0.23, cral:0.5')
    db.Get_Detected_Image([1, 3], 'tmp')
    
    db.Insert_Annotation_Data(image_path='123.png', anno_image_path='123.png', yolo_anno_path='anno.txt')
    
    db.Get_Annotation_Data([1], 'tmp')
    
    # db.Delete_Annotation_Data([1])
    # db.Delete_Detected_Image_Data([1, 3])